...
from elisa import dispatcher, LOGGER, OWNER_ID, MESSAGE_DUMP
from elisa.__main__ import DATA_IMPORT
...
# from elisa.modules.rules import get_rules
import elisa.modules.sql.rules_sql as rulessql
# from elisa.modules.sql import warns_sql as warnssql
import elisa.modules.sql.blacklist_sql as blacklistsql
from elisa.modules.sql import disable_sql as disabledsql
# from elisa.modules.sql import cust_filters_sql as filtersql
# import elisa.modules.sql.welcome_sql as welcsql
import elisa.modules.sql.locks_sql as locksql
...
def export_data(update, context):
	...
	f=open("elisa{}.backup".format(chat_id), "w")
	...
	try:
		...
		pass
	context.bot.sendDocument(current_chat_id, document=open('elisa{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `elisa-Backup` is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
	os.remove("elisa{}.backup".format(chat_id)) # Cleaning file
