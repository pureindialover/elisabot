...
from elisa.modules.helper_funcs.admin_rights import (
    user_can_changeinfo,
    user_can_invite,
    user_can_pin,
    user_can_promote,
...
def permanent_pin_set(update, context):
	...
	if conn:
		...
		if update.effective_message.chat.type == "private":
			reply_to.update.effective_message("You can do this command in groups, not PM")
			return ""
		chat = update.effective_chat
		chat_id = update.effective_chat.id
		chat_name = update.effective_message.chat.title
		if update.effective_message.reply_to_message:
			...
			sreply_to.update.effective_message("Permanent pin has been disabled!")
			return
		else:
			get_permapin = sql.get_permapin(chat_id)
			text_maker = reply_to.update.effective_message("Successfully set permanent pin: `{}`").format(bool(int(get_permapin)))
			if get_permapin:
				if chat.username:
					old_pin = "https://t.me/{}/{}".format(chat.username, get_permapin)
				else:
