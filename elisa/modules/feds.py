...
from elisa.modules.helper_funcs.alternate import (
    send_action,
    send_message,
    typing_action,
)
from elisa.modules.helper_funcs.chat_status import is_user_admin
from elisa.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
...
def new_fed(update, context):
    ...
    if not fednam == "":
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @pureindialover if the problem persists."
            )
            return
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "elisa_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="elisa_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "elisa_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="elisa_fbanned_users.csv",
